...
export function PohodaImportModal({ onClose, onImported }: { onClose: () => void; onImported: () => void }) {
  ...
  const actionableCount = preview ? preview.newCount + preview.updatedCount + preview.warningCount : 0;
  ...
  const importFile = async () => {
    if (!file || !actionableCount) return;
  ...
  return <div className="modal-backdrop"><section className="pohoda-import-modal"><header><div><p className="eyebrow">POHODA XML</p><h2>Import objednávek</h2></div><button onClick={onClose} aria-label="Zavřít">×</button></header><div className="pohoda-import-body"><input ref={input} type="file" accept=".xml,text/xml,application/xml" hidden onChange={event => void selectFile(event.target.files?.[0])} /><button className={`pohoda-drop-zone ${dragging ? "dragging" : ""}`} onClick={() => input.current?.click()} onDragEnter={event => { event.preventDefault(); setDragging(true); }} onDragOver={event => event.preventDefault()} onDragLeave={() => setDragging(false)} onDrop={event => { event.preventDefault(); setDragging(false); void selectFile(event.dataTransfer.files[0]); }}><FileUp size={28} /><strong>{file?.name ?? "Přetáhněte sem XML soubor z Pohody"}</strong><span>{file ? "Kliknutím můžete vybrat jiný soubor" : "nebo klikněte a vyberte soubor"}</span></button>{busy && <div className="loader"><LoaderCircle className="spin" /> Zpracovávám XML…</div>}{error && <div className="error-banner">{error}</div>}{preview && <><div className="pohoda-import-summary"><span className="new"><CheckCircle2 /> {preview.newCount} nových</span><span className="updated"><CheckCircle2 /> {preview.updatedCount} aktualizací</span><span className="unchanged">{preview.unchangedCount} beze změny</span><span className="warning"><TriangleAlert /> {preview.warningCount} upozornění</span><span className="invalid"><XCircle /> {preview.errorCount} chyb</span></div><div className="pohoda-preview-table"><table><thead><tr><th>Objednávka</th><th>Zákazník</th><th>Datum</th><th>Hodnota</th><th>Výsledek</th></tr></thead><tbody>{preview.rows.map((row, index) => <tr key={`${row.externalId}-${index}`}><td><strong>{row.pohodaOrderNumber ?? row.externalId}</strong><small>{row.title}</small></td><td>{row.customerName}<small>{row.companyNumber ? `IČO ${row.companyNumber}` : "Bez IČO"}</small></td><td>{formatDate(row.orderedOn)}</td><td>{formatMoney(row.valueCzk)}</td><td><span className={`pohoda-row-status ${row.status.toLowerCase()}`}>{statusLabel(row.status)}</span>{row.message && <small>{row.message}</small>}</td></tr>)}</tbody></table></div></>}</div><footer><button className="secondary" onClick={onClose}>Zrušit</button><button className="primary" disabled={busy || !actionableCount} onClick={() => void importFile()}>{busy ? "Importuji…" : `Zpracovat ${actionableCount} objednávek`}</button></footer></section></div>;
}

function statusLabel(status: PohodaImportPreview["rows"][number]["status"]) {
  if (status === "New") return "Nová";
  if (status === "Updated") return "Aktualizace";
  if (status === "Unchanged") return "Beze změny";
