...
public sealed class OrderCommandServiceTests
{
    ...
    {
        var repository = new FakeOrderRepository { Validation = new(false, true, true, true, true) };
    ...
    private static CreateOrderRequest ValidRequest() => new() { Number = "BCG_20260001", Title = "Test", CustomerId = Guid.NewGuid(), ValueCzk = 100, WeightKg = 5, VolumeM3 = 1 };

    private sealed class FakeOrderRepository : IOrderReadRepository, IOrderWriteRepository
    {
        public OrderReferenceValidation Validation { get; init; } = new(true, true, true, true, true);
        ...
        public Task<OrderReferenceValidation> ValidateReferencesAsync(Guid customerId, Guid? contactId, Guid? warehouseId, Guid? carrierId, Guid? customsDeclarantId, CancellationToken cancellationToken) => Task.FromResult(Validation);
        ...
        public Task<Order?> GetOrderAsync(Guid id, CancellationToken cancellationToken) => Task.FromResult(AddedOrder);
        public void SetOriginalVersion(Order order, uint version) { }
        public void Remove(Order order) { }
        public Task<TransportQuote?> GetQuoteAsync(Guid orderId, Guid quoteId, CancellationToken cancellationToken) => Task.FromResult<TransportQuote?>(null);
        public void SetOriginalVersion(TransportQuote quote, uint version) { }
        public Task<bool> IsCarrierAsync(Guid carrierId, CancellationToken cancellationToken) => Task.FromResult(true);
        public Task ClearSelectedQuoteAsync(Guid orderId, Guid? exceptId, CancellationToken cancellationToken) => Task.CompletedTask;
        public void AddQuote(TransportQuote quote) { }
