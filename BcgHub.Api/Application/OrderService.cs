...
{
    public Task<PagedResult<OrderListItem>> GetListAsync(string? search, string sortBy, bool descending, int page, int pageSize, Guid? customerId, OrderSalesChannel salesChannel, CancellationToken cancellationToken) => repository.GetListAsync(search, sortBy, descending, Math.Max(1, page), Math.Clamp(pageSize, 1, 100), customerId, salesChannel, cancellationToken);
